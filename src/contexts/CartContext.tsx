...
type CartAction = 
  ...
  | { type: 'ADD_TO_CART_SUCCESS'; payload: CartResponse }
...
const cartReducer = (state: CartState, action: CartAction): CartState => {
  switch (action.type) {
    ...
    case 'ADD_TO_CART_SUCCESS':
      return { ...state, itemBeingAdded: false, cart: action.payload };
...
export const CartProvider: React.FC<CartProviderProps> = ({ children }) => {
  ...
  const addToCart = async (item: CartItemRequest) => {
    ...
    try {
      ...
      message.success(t('notifications.item_added'));
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    } catch (error) {
  ...
  const updateQuantity = async (itemId: string, quantity: number) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
      ...
      dispatch({ type: 'UPDATE_QUANTITY_FAILURE', payload: 'Failed to update quantity' });
  ...
  const removeItem = async (itemId: string) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
      ...
      dispatch({ type: 'REMOVE_ITEM_FAILURE', payload: 'Failed to remove item' });
